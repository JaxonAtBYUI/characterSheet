...
for(let i = 0; i <characters.length; i++){
  ...
  // 
  console.log(name);
  renderDashboard(template, characters[i])
}
function renderDashboard(template, item){
  ...
  copy.querySelector(".dashboard__item").href += item.id;
  ...
  copy.querySelector(".bg").innerHTML = item.background;
  copy.querySelector(".ac").innerHTML = item.armor_class;
  copy.querySelector(".initiative").innerHTML = item.iniative;
  copy.querySelector(".speed").innerHTML = item.speed;
